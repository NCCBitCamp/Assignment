...
ALTER TABLE PF_TEMP ADD CONSTRAINT PK_PNO PRIMARY KEY(PNO);

--4) COURSE_PROFESSOR 테이블에 CNO, PNO을 PK로 추가하세요.
ALTER TABLE COURSE_PROFESSOR ADD CONSTRAINT PK_CNO_PNO PRIMARY KEY(CNO, PNO); -- COURSE_PROFESSOR 없어서 쿼리만 짬
...
CREATE TABLE T_BOARD (
	...
	CONSTRAINT PK_T_BOARD_NO PRIMARY KEY(BOARD_NO)
);
...
CREATE TABLE T_BOARD_FILE (
	...
	CONSTRAINT PK_T_BOARD_NO_FILE_NO PRIMARY KEY(BOARD_NO, BOARD_FILE_NO)
);
...
CREATE TABLE T_SNS (
	SNS_NO NUMBER,
	SNS_NM VARCHAR2(20),
	CONSTRAINT PK_T_SNS_NO PRIMARY KEY(SNS_NO)
);
...
CREATE TABLE T_SNS_UPLOADED (
	SNS_NO NUMBER,
	SNS_UPL_NO NUMBER,
	CONSTRAINT PK_SNS_UPLOADED_NO PRIMARY KEY(SNS_NO, SNS_UPL_NO),
	CONSTRAINT FK_SNS_UPLOADED_NO FOREIGN KEY(SNS_NO)
		REFERENCES T_SNS(SNS_NO)
);
